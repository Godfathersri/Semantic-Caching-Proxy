...
async def generate_response(
    ...
    try:
        ...
        print(
            "Cache search completed. Result:",
            cache_result
        )

        if (
            cache_result
            and cache_result.score is not None
            and cache_result.score
            >= settings.QDRANT_SIMILARITY_THRESHOLD
        ):

            payload = (
                cache_result.payload or {}
            )
        ...
        return GenerateResponse(
            ...
            matched_prompt=(
                cache_result.payload.get(
                    "prompt"
                )
                if (
                    cache_result
                    and cache_result.payload
                )
                else None
            ),
